...
export function titleSimilarity(left: string, right: string): number {
  const leftWords = new Set(normalizeTitle(left).split(' ').filter((word) => word && !GENERIC_WORDS.has(word)));
  const rightWords = new Set(normalizeTitle(right).split(' ').filter((word) => word && !GENERIC_WORDS.has(word)));
...
export function titlesMatch(left: string, right: string): boolean {
  const leftWords = normalizeTitle(left).split(' ').filter((word) => word && !GENERIC_WORDS.has(word));
  const rightWords = normalizeTitle(right).split(' ').filter((word) => word && !GENERIC_WORDS.has(word));
  return leftWords.length >= 3 && rightWords.length >= 3 && titleSimilarity(left, right) >= 0.85;
}

export function scoreOpportunity(signals: DetectionSignal[], now = new Date()): number {
  const latest = Math.max(...signals.map((signal) => new Date(signal.discoveredAt).getTime()));
  ...
  const sources = Math.min(25, (new Set(signals.map((signal) => signal.researchSourceId)).size - 1) * 12);
