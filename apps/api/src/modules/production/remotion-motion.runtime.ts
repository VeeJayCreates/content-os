...
const Scene=({scene})=>{const frame=useCurrentFrame(),motion=scene.motion||{cameraMotion:'static',transition:'cut',overlays:[],map:null};const transitionStyle=transition(motion.transition,frame,scene.durationInFrames);const media=scene.media?(scene.mediaType==='video'?<OffthreadVideo src={staticFile(scene.media)} muted style={{width:'100%',height:'100%',objectFit:'contain'}}/>:<Img src={staticFile(scene.media)} style={{width:'100%',height:'100%',objectFit:'contain'}}/>):null;return <AbsoluteFill style={{overflow:'hidden',...transitionStyle}}><AbsoluteFill style={{transform:camera(motion.cameraMotion,frame,scene.durationInFrames)}}>{media}</AbsoluteFill><MapChoreography map={motion.map} duration={scene.durationInFrames}/>{motion.overlays.map((overlay,index)=><Overlay key={index} overlay={overlay}/>)}<Audio src={staticFile(scene.audio)}/></AbsoluteFill>};
