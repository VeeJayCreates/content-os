...
import { AudioGenerationStatus, VideoCompositionAssetStrategy, VideoCompositionPlanStatus, VideoRenderInputFailureCode, VideoRenderInputManifestStatus, VisualAssetManifestStatus, validateInternalVisualSpecification } from '@content-os/contracts';
import { AudioGenerationRepository, VideoCompositionRepository, VideoMotionRepository, VideoRenderInputRepository, VisualAssetRepository } from '@content-os/storage';
...
export class VideoRenderInputService{
 constructor(private readonly compositions:VideoCompositionRepository,private readonly audio:AudioGenerationRepository,private readonly visuals:VisualAssetRepository,private readonly materialization:MediaMaterializationService,private readonly manifests:VideoRenderInputRepository,@Optional() private readonly motions?:VideoMotionRepository){}
 ...
 async prepare(contentScriptId:string){
  ...
  const motionPlan=await this.motions?.findByContentScriptId(contentScriptId);let motionByCompositionScene=new Map<string,any>();let motionIdentity={videoMotionPlanId:null as string|null,videoMotionInputHash:null as string|null};
  if(motionPlan?.status==='ready'&&motionPlan.videoCompositionPlanId===composition.id&&motionPlan.compositionInputHash===composition.inputHash){
   try{validateMotionScenes(motionPlan.scenes,composition.scenes.map((scene:any)=>scene.audioDurationMs));if(motionPlan.scenes.some((scene:any,index:number)=>scene.sceneIndex!==index||scene.compositionSceneId!==composition.scenes[index]?.id||scene.plannedSceneId!==composition.scenes[index]?.plannedSceneId))throw new Error('identity');motionByCompositionScene=new Map(motionPlan.scenes.map((scene:any)=>[scene.compositionSceneId,scene]));motionIdentity={videoMotionPlanId:motionPlan.id,videoMotionInputHash:motionPlan.inputHash};}catch{motionByCompositionScene=new Map();}
  }
  ...
  for(let index=0;index<composition.scenes.length;index++){
   ...
   const motion=motionByCompositionScene.get(scene.id);const common={compositionSceneId:scene.id,plannedSceneId:scene.plannedSceneId,startMs:scene.audioStartMs,endMs:scene.audioEndMs,durationMs:scene.audioDurationMs,audioSegmentId:segment.id,audioPath:segment.audioPath,assetStrategy:scene.assetStrategy,selectedCandidateId:scene.selectedCandidateId,candidateIdentityHash:scene.candidateIdentityHash,motion:motion?{cameraMotion:motion.cameraMotion,transition:motion.transition,overlays:motion.overlays,map:motion.map}:null};
   if(scene.assetStrategy===VideoCompositionAssetStrategy.NO_ASSET){if(scene.selectedCandidateId||scene.candidateIdentityHash||scene.mediaAssetId)return this.fail(composition,VideoRenderInputFailureCode.COMPOSITION_STALE,`No-asset scene identity is invalid at scene ${index}`);let internalVisual;try{internalVisual=validateInternalVisualSpecification(scene.internalVisual);}catch{return this.fail(composition,VideoRenderInputFailureCode.COMPOSITION_STALE,`Internal visual specification is invalid at scene ${index}`);}scenes.push({...common,internalVisual,mediaAssetId:null,mediaType:null,mimeType:null,storageProvider:null,storageKey:null,checksum:null});continue;}
   ...
   scenes.push({...common,internalVisual:null,mediaAssetId:asset.id,mediaType:asset.mediaType,mimeType:asset.mimeType,storageProvider:asset.storageProvider,storageKey:asset.storageKey,checksum:asset.checksum});
  }
