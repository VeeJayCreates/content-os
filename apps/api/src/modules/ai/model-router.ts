...
export class ModelRouter {
  route(task: AiTask): AiRoute {
    switch (task) {
      ...
      case AiTask.CONTENT_PACKAGE_GENERATION:
        return { task, provider: this.cloudProvider(), model: process.env.AI_CONTENT_PACKAGE_MODEL ?? process.env.AI_SCRIPT_GENERATION_MODEL ?? process.env.OPENAI_MODEL ?? null, capability: AiCapability.STRUCTURED_GENERATION, timeoutMs: CONTENT_ANGLE_TIMEOUT_MS, costMode: 'configured', fallback: null };
      case AiTask.SCENE_PLANNING:
        return { task, provider: this.cloudProvider(), model: process.env.AI_SCENE_PLANNING_MODEL ?? process.env.AI_CONTENT_PACKAGE_MODEL ?? process.env.AI_SCRIPT_GENERATION_MODEL ?? process.env.OPENAI_MODEL ?? null, capability: AiCapability.STRUCTURED_GENERATION, timeoutMs: CONTENT_ANGLE_TIMEOUT_MS, costMode: 'configured', fallback: null };
