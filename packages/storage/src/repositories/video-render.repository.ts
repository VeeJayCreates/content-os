...
export class VideoRenderInputRepository{
 private present(manifest:typeof videoRenderInputManifests.$inferSelect,scenes:(typeof videoRenderSceneInputs.$inferSelect)[]){return {...manifest,scenes:scenes.map(scene=>({...scene,motion:scene.motionJson?JSON.parse(scene.motionJson):null}))};}
 async findByIdentity(id:string,inputHash:string){const manifest=(await db.select().from(videoRenderInputManifests).where(and(eq(videoRenderInputManifests.id,id),eq(videoRenderInputManifests.inputHash,inputHash))))[0];if(!manifest)return undefined;const scenes=await db.select().from(videoRenderSceneInputs).where(eq(videoRenderSceneInputs.manifestId,manifest.id)).orderBy(videoRenderSceneInputs.sceneIndex);return this.present(manifest,scenes);}
